...
$profit_dir/profit <<_Eod_ >> rmsd_shl2_bs.tmp
     multi $list1
     atom $atoms
     zone A17-A23
     zone B\-17-B\-17
...
set first_no = `cat rmsd_shl2_bs.tmp | grep -n iter | tail -1 | awk '{print $1}' | sed 's/://g'`
set last_no = `wc -l rmsd_shl2_bs.tmp | awk '{print $1}'`
@ dif = $last_no - $first_no + 1 
tail -`echo $dif` rmsd_shl2_bs.tmp | grep -v iter |awk '{print $2}' | sed '/^$/d' > rmsd_shl2_bs.disp_1
...
set last_no = `wc -l rmsd_shl2_bs.tmp | awk '{print $1}'`
@ dif = $last_no - $first_no + 1 
tail -`echo $dif` rmsd_shl2_bs.tmp | grep -v iter | awk '{print $2}' | sed '/^$/d' > rmsd_shl2_bs.disp_2

rm -rf *tmp

paste listaa rmsd_shl2_bs.disp_1 > rmsd_shl2.pdb
paste listab rmsd_shl2_bs.disp_2 >> rmsd_shl2.pdb

# please write your reference pdb file name here
grep -v dna_sox11_shl2_ref.pdb rmsd_shl2.pdb > a
mv a rmsd_shl2.pdb

# change the name of your rmsd file name "Sox11_shl2_sim1.rmsd-sorted"
sort -nk2 rmsd_shl2.pdb > Sox11_shl2_sim1.rmsd-sorted

rm rmsd_shl2_bs.disp_* rmsd_shl2.pdb